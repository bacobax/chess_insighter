...
def _compute_global_priors(
    ...
    """Return (median_structure_diversity, median_final_structure_entropy).
    ...
    richer_rows = [
        row for row in fv_rows
        if int(row.get("line_count") or 0) >= 2
    ]
    prior_rows = richer_rows or list(fv_rows)
    diversities = sorted(
...
def get_opening_study_tree_children(
    ...
    ``engine`` is accepted for backward compatibility but is intentionally unused:
    interactive expansion treats the opening feature CSV as the source of truth.
